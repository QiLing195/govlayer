...
| `GOVLAYER_SEMANTIC_MIN_SCORE` | `0.55` | 相似度下限；**没有任何单一值适用，必须逐库标定**（见下） |
| `GOVLAYER_RETRIEVAL` | `auto` | `auto`=LLM 优先；`local`=**检索本地化**；`llm`=强制 LLM |

三个必须知道的行为点：

1. **相似度下限必须逐库标定，没有任何单一值适用**。向量检索永远会返回 top-k，哪怕问题与
   知识库毫无关系。这个值的历史很说明问题：最初是**猜的**（`0.25`）→ 太低，问"公司食堂
   几点开饭？"也捞回 3 条，系统据此说出"制度中有相关规定，但超出你的角色权限范围"——
   一句会被当场戳穿的假话；后来在**单份语料**上标成 `0.51`；**四语料实测证明它不通用**：

   | 数据集 | 条数 | 远域组建议阈值 | 相关召回 |
   |---|---|---|---|
   | `employee_rules` | 7 | 0.499 | 94% |
   | `registration` | 7 | 0.535 | **57%** |
   | `kaohe` | 50 | 0.557 | 95% |
   | `renshi` | 43 | 0.567 | 75% |

   建议值跨度 0.499~0.567、**没有交集**。现取 `0.55` 作兜底，并在启动时提示"未标定"。

   ⚠️ **比阈值更要紧的两条**（详见 `V63_RETRIEVAL_THRESHOLD_REPORT.md`）：

   - **阈值挡不住"问的是本领域但制度没规定"的问题**。实测 `kaohe` 上
     "考核不合格会影响退休待遇吗？"得分 **0.6869**，深埋在相关组分部内部；
     挡住它需要阈值 0.692，代价是相关召回只剩 **7/20**。判断这类问题
     **必须靠覆盖判定，不是靠相似度**。
   - **检索的绝对准确率随语料增大而下降**：top-1 命中从 7 条语料的 71%
     降到 50 条语料的 **25%**（第11~14条结构几乎相同，模型难区分）。
     所以**不要给单一答案**——必须返回 top-k 并配合覆盖判定。

   落地含义：
   - `GOVLAYER_RETRIEVAL=local`（检索本地化）**仍需 LLM 做覆盖判定**；
   - **纯本地（连 LLM 都没有）时，覆盖判定退化为关键词探针**，
